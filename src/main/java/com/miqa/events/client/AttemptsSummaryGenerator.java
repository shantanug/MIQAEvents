...
public class AttemptsSummaryGenerator implements Job {
...
//		gen.getAnswerChoiceToOptionLabel();
	...
	public boolean generateSummary() throws SQLException, ParseException {
		System.out.println("****************BATCH BEGINS ************************" + new Date());
		/*String startTime = readStartTime();
		Date startD = getDate(startTime);
		*/
		try {
		...
		if(startId < maxId) ansChoiceToOptionLabel = getAnswerChoiceToOptionLabel();

		while (startId > 0 && startId < maxId) {
//			if(testRun > 10) break;
//				testRun++;
			endId = startId + 25000 <= maxId ? startId+25000:maxId;
//			System.out.println("before getting attempts " +startId + " - "+ endId);
...
//		String dateStr = dateToString(startD);
		...
		closeConnection();
		System.out.println("****************BATCH COMPLETED ************************"+new Date());
		return true;
	...
	private void updateStartId(long endId) throws SQLException {
		...
		String sql = "update miqa_setting set value = ? where name = 'attempt_summary_curr_id'";
	...
	private long getMaxId() throws SQLException {
		String sql = "select max(id) as max_id from miqa_quiz_session";
...
//		conn.close();
	...
	private long getStartId() throws SQLException {
		String sql = "select value from miqa_setting where name = 'attempt_summary_curr_id' ";
		Connection conn = getConnection();
		java.sql.PreparedStatement ps = conn.prepareStatement(sql);
		ResultSet rs = ps.executeQuery();
		rs.next();
...
//		conn.close();
	...
	private void updateStartTime(String dateStr) throws SQLException {
		Connection conn = getConnection();
		String sql = "update miqa_setting set attempt_summary_start_time = ?";
		PreparedStatement ps = conn.prepareStatement(sql);
...
//        startTime = "2014-10-05T15:23:01";
	...
	private Map<Integer, Integer> getAnswerChoiceToOptionLabel() throws SQLException {
		
		String sql = "SELECT id, question_id, @choice:=CASE WHEN @question <> question_id "
				+ "THEN 0 ELSE @choice+1 END AS rn, @question:=question_id AS clset FROM "
				+ "(SELECT @choice:= -1) s, (SELECT @question:= -1) c, "
				+ "(SELECT * FROM miqa_answer_choice ORDER BY question_id, id) t";
...
//			System.out.println(" choiceId = "+choiceId + " rank = "+rank + " questionId = "+questionId);
	...
	private String readStartTime() throws SQLException {
		String sql = "select value from miqa_setting where name = 'attempt_summary_start_time' ";
		Connection conn = getConnection();
		java.sql.PreparedStatement ps = conn.prepareStatement(sql);
		ResultSet rs = ps.executeQuery();
		rs.next();
...
//		conn.close();
	...
	private void createSummaryVO(List<Attempt> attempts,
			Map<Integer, Integer> ansChoiceToOptionLabel, Date currTime) throws SQLException {
...
//					+ attempt.isFirstAttempt();
			String key = createKey(questionId, majorVersion, attempt.isFirstAttempt());
			...
			if (oldSummary == null) {
				...
				/*String oldSummKey = oldSummary.getQuestionId() + "_"
						...
						*/
				String oldSummKey = createKey(oldSummary.getQuestionId(), oldSummary.getMajorVersion(), oldSummary.isFirstAttempt());
	...
	private List<AttemptSummary> getOldAttemptSummary() throws SQLException {
		...
		*/
		Connection conn = getConnection();
		String sql =  "select * from miqa_attempt_summary ";
		PreparedStatement ps = conn.prepareStatement(sql);
		ResultSet rs = ps.executeQuery();
		...
		while(rs.next()) {
			...
			String qbName = rs.getString("question_bank_name");
			int questionId = rs.getInt("question_id");
			String questionName = rs.getString("question_name");
			
			String  qtName = rs.getString("quiz_template_name");
...
//		System.out.println("got option as "+option);
		/*if(option  >= 5) {
			System.out.println("Error option greater than E  ");
			System.exit(1);
		}
		*/
		switch (option) {
	...
	private void updateDB(Map<String, AttemptSummary> keyToAttemptSummary, Date currTime) throws SQLException {
		Iterator<String> iter = keyToAttemptSummary.keySet().iterator();
		Connection conn = getConnection();
		String 	updateSQL = "update miqa_attempt_summary set question_id = ? , question_name=?,correct_count = ? , incorrect_count = ?, "
				+ "major_version = ? , minor_version = ?  , question_bank_name = ? ,quiz_template_name = ? ,choice_a_count = ? , "
				+ "choice_b_count = ? , choice_c_count = ? ,choice_d_count = ? , choice_e_count =  ? ,total_count=? ,  last_modified_date  = ? ,"
				+ "is_first_attempt=? ,difficulty_level = ? where id = ?   ";
		
		String insertSQL = "insert into miqa_attempt_summary  (question_id,question_name,correct_count,incorrect_count,major_version,minor_version"
				+ ",question_bank_name,quiz_template_name,choice_a_count,choice_b_count,choice_c_count,choice_d_count,choice_e_count,total_count,last_modified_date,"
				+ "is_first_attempt,difficulty_level) 	values (?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?,?) ";
...
//		if(ps != null) conn.close();
	...
	private void setPreparedStatement(AttemptSummary summary,
		...
		ps.setString(7, summary.getQuestionBankName());
		ps.setString(8,summary.getQuizTemplateName());
		ps.setInt(9,summary.getChoiceACount());
		ps.setInt(10,summary.getChoiceBCount());
		ps.setInt(11,summary.getChoiceCCount());
		ps.setInt(12,summary.getChoiceDCount());
		ps.setInt(13,summary.getChoiceECount());
		ps.setInt(14, summary.getTotalCount());
		ps.setTimestamp(15,new Timestamp(currTime.getTime()));
		ps.setBoolean(16,summary.isFirstAttempt());
		ps.setString(17, summary.getDifficultyLevel());
//		ps.setBoolean(18,summary.isQuestionEnabled());
		if(summary.getId() > 0  ) {
//			System.out.println("	ary id "+ summary.getId());
			ps.setLong(18,summary.getId());
		}
	...
	public List<Attempt> getRecords(long startId,long endId) throws Exception {
		...
		*/
		...
		System.out.println(" getting records for ids between  "+startId + " and "+endId);
		
		String sql = "select qs.* ,q.status as question_status, q.name as question_name,qb.name as question_bank_name,qt.name as quiz_template_name from miqa_quiz_session qs , miqa_question q, miqa_question_bank qb , miqa_quiz_template qt where "
				+ "qs.id >= ? and qs.id <= ? and qs.question_id = q.id and qs.answer_chosen > 0 and q.question_bank_id = qb.id and qb.quiz_template_id = qt.id"; 
		Connection conn = getConnection();
		PreparedStatement pstmt = conn.prepareStatement(sql);
		
//		java.sql.Timestamp sqlStartDate = new java.sql.Timestamp(startId);
//		java.sql.Timestamp sqlEndDate = new java.sql.Timestamp(endTime.getTime());
...
//		System.out.println("query executed  ");
		...
		while(rs.next()) {
			...
			attempt.setQuizTemplatteName(rs.getString("quiz_template_name"));
			attempt.setQuestionBankName(rs.getString("question_bank_name"));
...
//			attempt.setQuestionEnabled(questionEnabled);
			...
			if(attempt.getQuestionId() == 0) throw new Exception("Invalid question id "+attempt.getQuestionId());
			retVal.add(attempt);
